...
class FollowingListResource(Resource):
    ...
    def post(self):
        ...
        if ret > 0:
            ...
            cache_statistic.UserFollowingCountStorage.incr(g.user_id)
            cache_statistic.UserFansCountStorage.incr(target)
            cache_user.UserRelationshipCache(g.user_id).clear()

        # 发送关注通知
        # 借助 socketio manager 将推送通知的任务添加到rabbitmq中
...
class FollowingResource(Resource):
    ...
    def delete(self, target):
        ...
        if ret > 0:
            ...
            cache_statistic.UserFollowingCountStorage.incr(g.user_id, -1)
            cache_statistic.UserFansCountStorage.incr(target, -1)
            cache_user.UserRelationshipCache(g.user_id).clear()
